...
void GateSourceTPSPencilBeam::GenerateVertex( G4Event *aEvent ) {
  ...
  if (mSortedSpotGenerationFlag){
    while ( (mCurrentSpot<mTotalNumberOfSpots) && (mNbIonsToGenerate[mCurrentSpot] <= 0) ){
      ...
      mCurrentSpot++; //Enter each spot in succession, beginning with the first
  ...
  mPencilBeam->GenerateVertex(aEvent); //simulate an ion
  if (mSortedSpotGenerationFlag){
    --mNbIonsToGenerate[mCurrentSpot]; //remove one ion from the spot (prefilled)
  }
}
//---------GENERATION - END-----------------------
